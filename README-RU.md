...
Cloudflare анонсирует свой anycast из региона клиента, поэтому Google геолоцирует
выход WARP обратно туда, где пользователь находится на самом деле. Замер с
финского VPS — YouTube публикует вердикт Google прямо в исходнике страницы полем
`"GL":"XX"`, это один запрос и никаких ключей:

| Выход | Google считает, что вы в |
|---|---|
| Psiphon, регион DE | `DE` |
| Cloudflare WARP | `RU` |
| Собственный адрес VPS | `FI` |

Дальше сервисы с региональным гейтом отказывают со словами «недоступно в вашей
стране» — с их стороны честно, с вашей крайне непонятно, и сменой endpoint'а WARP
не лечится. Psiphon же отвечает честно: запрос JP, NL или DE даёт ровно `JP`, `NL`,
`DE`.

**Правильная страна — условие необходимое, но не достаточное.** Выход может
находиться где надо и всё равно получить отказ из-за репутации самого адреса, а
этого не видит ни одна проверка без авторизации. Для такого случая и существует
`HEALTH_CMD`.
